...
class _LoginScreenState extends State<LoginScreen> {
  ...
  Widget _buildBody(LoginViewModel viewModel) {
    Widget loginButton = MinimalElevatedButton(
      label: 'LOG IN',
      onPressed: () {
    ...
    if (viewModel.state == LoginViewState.loading) {
      loginButton = CircularProgressIndicator(
        color: AppColors.accentColor,
      );
    }

    if (viewModel.state == LoginViewState.error) {
  ...
  Widget build(BuildContext context) => BlurredBackground(
        ...
        child: Scaffold(
          ...
          body: ChangeNotifierProvider<LoginViewModel>(
            create: (_) => context.read<Injector>().loginViewModel,
